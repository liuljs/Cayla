...
function process(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        // 規格
        if (data[i].Spec !== "" && data[i].Spec !== null) {
            spec = `<span>${data[i].Spec}</span>`
        } else {
            spec = `<span> - </span>`
        };
        // 停售
        ...
        html += `
            <tr data-num="${data[i].Id}">
                ...
                <td data-title="商品規格">${spec}</td>
                <td data-title="商品定價" class="text-right"><span class="priceMark">${thousands(data[i].Price)}</span></td>
                <td data-title="商品總數量" class="text-right">${thousands(sells)}</td>
                <td data-title="售出總數量" class="text-right">${thousands(data[i].sell_qty)}</td>
                            ...
                            <tr data-num="${callBackData[i].Id}">
                                ...
                                <td data-title="狀態">
                                    <div class="custom-control custom-switch">
                                        <input type="checkbox" class="custom-control-input edit_status inputStatus" id="stockSwitch${i}" value="${callBackData[i].Enabled}" disabled>
                                        <label class="custom-control-label" for="stockSwitch${i}">開啟</label>
                                    </div>
                                </td>
                            </tr>
                        `;
