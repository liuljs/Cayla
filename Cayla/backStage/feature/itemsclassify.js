...
function process(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        for (let j = 0; j < data[i].SubCategories.length; j++) {
            secs += `
                <tr data-num="${data[i].SubCategories[j].Id}">
                    <td data-title="名稱" class="cls_name">
                        <div class="input-group">
                            <input type="text" class="form-control inputStatus edit_secName" value="${data[i].SubCategories[j].Name}" disabled>
                            <input type="hidden" class="edit_minCls" value='${JSON.stringify(data[i].SubCategories[j].SubCategories)}'>
                        </div>
                    </td>
                    <td data-title="狀態" class="cls_status" data-num="${i}">
                        <div class="custom-control custom-switch">
                            <input type="checkbox" class="custom-control-input inputStatus edit_status edit_secStatus" id="${data[i].SubCategories[j].Id}secSwitch${i}" value="${data[i].SubCategories[j].Enable}" disabled>
                            <label class="custom-control-label" for="${data[i].SubCategories[j].Id}secSwitch${i}">開啟</label>
                        </div>
                    </td>
                ...
                <div class="card-header">
                    ...
                    <div class="d-flex flex-wrap my-2">
                        ...
                        <div class="form-group d-flex align-items-center justify-content-end py-1 pr-0 col-md-4">
                            ...
                            <div class="custom-control custom-switch">
                                <input type="checkbox" id="primSwitch${i}" class="custom-control-input inputStatus edit_status edit_priStatus" value="${data[i].Enable}" disabled>
            ...
            if (menuAuth[authParent.indexOf("PM")].ACT_EDT == "Y") {
                ...
                $(document).on('click', '.btnEditSec', function (e) {
                    ...
                    $('.btnSaveSec').on('click', function (e) {
                        ...
                        if (check == true) {
                            console.log(clsMinArr.concat(clsAddArr, clsDelArr))
                            let dataObj = {
                                "id": id,
                ...
                html = `
                    <div class="modal-header">
                        <h3 class="modal-title d-flex" data-num="${trz.data('num')}" data-name="${trz.find('.edit_secName').val()}">
                            <span>次分類</span>
                            <span class="name-set text-primary">${trz.find('.edit_secName').val()}</span>
