...
function process(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        // 商品狀態
        if (data[i].Order_Status_Id == "11") {
            status = "";
            if (data[i].Pay_Type_sId == "01") {
                status = `
                    <div>
                        <span class="status" data-num="${data[i].Order_Status_Id}">${data[i].Order_Status}</span>
                ...
                status = `
                    <div>
                        <span class="status" data-num="${data[i].Order_Status_Id}">${data[i].Order_Status}</span>
                        <span class="status-note">買家尚未付款（在 ${data[i].Pay_End_Date} 前）</span>
                    </div>
                `;
            };
        } else {
        ...
        html += `
            <tr data-num="${data[i].Id}">
                <td data-title="訂單編號">${data[i].Id}</td>
                <td data-title="訂單日期" class="text-left">
                    <div>
                        <span class="mr-1">${data[i].Purchase_Date.split(' ')[0]}</span>
                        <span>${data[i].Purchase_Date.split(' ')[1]}</span>
                    </div>
                </td>
                <td data-title="訂單金額" class="text-right"><span class="priceMark amt">${thousands(parseInt(Number(data[i].Order_Total) + Number(data[i].Delivery_Fee)))}</span></td>
                <td data-title="付款方式" class="text-center">${data[i].Pay_Type}</td>
                <td data-title="狀態" class="order-status">${status}</td>
                <td data-title="預估出貨時間" class="text-left">
                    <span class="mr-1">${data[i].Delivery_Date.split(' ')[0]}</span>
                </td>
                <td data-title="配送方式" class="text-center">${data[i].Delivery_Type}</td>
...
$().ready(function () {
    ...
    orderLiv.on('click', function (e) {
        ...
        if (odrSearchs.val() !== "" || dateSearch.val() !== "") {// 點擊時，清空搜尋欄位
        ...
        if (subOrderStatus.html() !== "") {
            ...
            subOrderStatus.find('ul li').eq(0).find('a').trigger('click');
        } else {
            // 判斷點擊時
            if (clsNum !== clsRcd) {
                pageRcd = ""; // 重置頁碼紀錄
    ...
    btnSearch.on('click', function (e) {
        ...
        if (odrSearchs.val() !== "") {
            ...
            clsRcd = "", cls1Rcd = "", pageRcd = ""; // 使用搜尋，清空目前記錄的篩選 && 每次使用都要清除頁碼紀錄
            getTotalPages(dataObj).then(res => {
                if (res !== 0) {
                    pageLens = res; // 目前總頁數
                    paginations.find('div').html(curPage(current, pageLens, pageCount));
