...
function process(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        html += `
            <div class="card" data-num="${data[i].Id}">
                <div class="card-header">
                    <div class="form-group d-flex justify-content-between align-items-center">
                        <h4 class="text-info mb-0 faqNum"">
                            <span>問答編號 ${data[i].Id}</span>
                        </h4>
                        <div class="custom-control custom-switch">
                            <input type="checkbox" class="custom-control-input edit_status inputStatus" id="${data[i].Id}faqStatus${i}" value="${data[i].Enabled}" disabled>
                            <label class="custom-control-label" for="${data[i].Id}faqStatus${i}">開啟</label>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                        <div class="input_wrap d-flex flex-column">
                            <div class="form-group d-flex align-self-end py-0">
                            ...
                            <div class="form-group">
                                <input type="text" class="form-control edit_qst inputStatus" placeholder="問題" value="${data[i].Question}" disabled>
                                <textarea type="text" class="form-control mt-2 edit_ans inputStatus" placeholder="回答" disabled>${data[i].Asked}</textarea>
                            </div>    
                        </div>
                        <div class="form-group">
