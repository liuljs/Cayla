...
// Cart Totals 
let total, amt, totalQty = $('.total-qty'), totalAmt = $('.total-amount'), totalFee = $('.total-fee'), totals = $('.order-total');
let freeSet = $('.freeSet');
let freeField = $('<span>運費</span><span class="notes">免運費</span>');
let feeField = $('<span>運費</span><span class="priceMark total-fee"></span>');
...
$().ready(function () {
    ...
    tabs.find('input[type="radio"]').on('change', function () {
        let tabContent = $(this).parents('.group-full').find('.tab-contents');
    ...
    getPageDatas(dataObj).then(res => {
        ...
        if (res !== null) {
            ...
            cloneStatus.on('change', function () {
                if ($(this).prop('checked') == true) {
                    if (purCustomer.val() !== "") {
                        ...
                        $(`input[name="devGender"][value="${purGender.val()}"]`).prop('checked', true);
                    };
                    if (purCell.val() !== "") {
            ...
            // alert("錯誤訊息 " + cartInfo.status + "：您的連線異常，請重新登入！");
            getLogout();
        };
    }, rej => {
        if (rej == "NOTFOUND") {
